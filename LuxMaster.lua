...
function OnLoad()
	...
	Menu()
	SSpells = SumSpells()
	print("<b><font color=\"#FF0000\">Lux Master:</font></b> <font color=\"#FFFFFF\">Good luck and give me feedback!</font>")
	if _G.MMA_Loaded then
		print("<b><font color=\"#FF0000\">Lux Master:</font></b> <font color=\"#FFFFFF\">MMA Support Loaded.</font>")
	end	
	if _G.AutoCarry then
		print("<b><font color=\"#FF0000\">Lux Master:</font></b> <font color=\"#FFFFFF\">SAC Support Loaded.</font>")
	end
end
...
function Menu()
	...
	MenuLux.jsConfig:addParam("JSEnabled", "Use Jungle Steal", SCRIPT_PARAM_ONKEYTOGGLE, false, string.byte("H"))
	...
	MenuLux.jsConfig:permaShow("JSEnabled")
...
function Farm()
	EnemyMinions:update()
	local QMode =  MenuLux.farm.QF
	local EMode =  MenuLux.farm.EF
	for i, minion in pairs(EnemyMinions.objects) do
		...
		if EMode == 3 then
			...
			CastSpell(_E)
		elseif EMode == 2 then
			if minion ~= nil and GetDistance(minion) < E.range then
...
function OnDraw()
	if MenuLux.drawConfig.DST and MenuLux.comboConfig.ST then
		if SelectedTarget ~= nil and not SelectedTarget.dead then
			DrawCircle2(SelectedTarget.x, SelectedTarget.y, SelectedTarget.z, 100, RGB(MenuLux.drawConfig.DQRC[2], MenuLux.drawConfig.DQRC[3], MenuLux.drawConfig.DQRC[4]))
		end
	end
	...
	if MenuLux.drawConfig.DQR and Q.Ready() then			
		DrawCircle2(myHero.x, myHero.y, myHero.z, Q.range, RGB(MenuLux.drawConfig.DQRC[2], MenuLux.drawConfig.DQRC[3], MenuLux.drawConfig.DQRC[4]))
	end
	if MenuLux.drawConfig.DWR and W.Ready() then			
		DrawCircle2(myHero.x, myHero.y, myHero.z, W.range, RGB(MenuLux.drawConfig.DWRC[2], MenuLux.drawConfig.DWRC[3], MenuLux.drawConfig.DWRC[4]))
	end
	if MenuLux.drawConfig.DER and E.Ready() then			
		DrawCircle2(myHero.x, myHero.y, myHero.z, E.range, RGB(MenuLux.drawConfig.DERC[2], MenuLux.drawConfig.DERC[3], MenuLux.drawConfig.DERC[4]))
	end
...
function StealJungle()
	KSMinions:update()
	if MenuLux.jsConfig.JSEnabled then
		...
		if monster and ValidTarget(monster, R.range) then
			local pred = VP:GetPredictedHealth(monster, 1, 0)
			local RDMG = getDmg("R", monster, myHero)
			if pred < RDMG then
...
function CastQ2(unit)
	if unit and Q.Ready() and ValidTarget(unit) then
		...
		if MenuLux.prConfig.pro == 3 then
			local unit = DPTarget(unit)
			local LuxQ = LineSS(Q.speed, Q.range, Q.width, Q.delay*1000, 1)
